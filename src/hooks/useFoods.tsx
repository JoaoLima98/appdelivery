...
export const FoodsProvider = ({ children }: { children: ReactNode }) => {
  ...
  async function getAll(): Promise<FoodsProps[]> {
    try {
      const query =
        "SELECT id, name, price, time, delivery, rating, image, restaurantId FROM foods";
  ...
  return (
    <FoodsContext.Provider
      value={{ foods, remove, searchByName, isPending, getAll }}
    >
      {children}
    </FoodsContext.Provider>
  );
